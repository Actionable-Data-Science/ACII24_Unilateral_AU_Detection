...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, image_shape, increase_area=0.1, output_video="cropped.mp4"):
    ...
    return f'ffmpeg -i {inp} -ss {start} -t {time} -filter:v fps=20 "crop={w}:{h}:{left}:{top}, scale={scale}" {output_video}'


def compute_bbox_trajectories(trajectories, fps, frame_shape, input_video, output_video="cropped.mp4"):
    commands = []

    for i, (bbox, tube_bbox, start, end) in enumerate(trajectories):
        # print(start,end)
        # if (end - start) > 150:
        command = compute_bbox(start, end, fps, tube_bbox, frame_shape, inp=input_video, image_shape=(256,256), increase_area=0.1,output_video=output_video)
...
def process_video_function(input_video, output_video):
    device = 'cpu' #if args.cpu else 'cuda'
    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, flip_input=False, device=device)
    ...
    try:
        for i, frame in tqdm(enumerate(video)):
            ...
            commands += compute_bbox_trajectories(not_valid_trajectories, fps, frame_shape, input_video,output_video)
            trajectories = valid_trajectories
            ...
            for bbox in bboxes:
                ...
                for trajectory in trajectories:
                    tube_bbox = trajectory[0]
                    current_intersection = bb_intersection_over_union(tube_bbox, bbox)
                    if intersection < current_intersection and current_intersection > 0.25:
                        intersection = bb_intersection_over_union(tube_bbox, bbox)
                        current_trajectory = trajectory
    ...
    commands += compute_bbox_trajectories(trajectories, fps, frame_shape, input_video, output_video)
    return commands
